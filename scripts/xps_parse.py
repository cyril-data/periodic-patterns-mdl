import re, datetime, glob, sys, numpy
import pdb
...
def timedeltastr_to_num(dT):
    tmp = re.match("(?P<days>\d+ days?, )?(?P<hours>\d?\d):(?P<mins>\d\d):(?P<secs>\d\d)\.(?P<ms>\d+)$", dT)
    if tmp is not None:
...
def parse_log_times(fn):
    ...
    with open(fn) as fp:
        for line in fp:
            if re.match("\[TIME\]", line):
                ...
                tmp = re.match("\t\S+ \[\d+\] \(\|O\|=(?P<ev_O>\d+) f=[0-9\.]+ dT=\d+\)", line)
                if tmp is not None:
    ...
    for line in time_lines + inter_lines:
        ...
        if tmp is not None:
            ...
            tmp = re.match("(\[[A-Z]+\] )?(?P<what>\S+) selection done in (?P<time>[0-9:\.]+) at", line)
            if tmp is not None:
                what = tmp.group("what")
                if what not in selects:
                    selects[what] = {}
...
def format_stats_head(add_stats=[], mks=[]):
    ...
    for (short, imain, isec) in INTERMS:
        entries.extend(["%s_%s" % (short, main_stat) for main_stat in MAIN_STATS])
...
for fn in sorted(glob.glob(XPS_REP + XPS_SUB + LOG_MATCH)):
    ...
    if tmp is not None:
        ...
        fo.write(format_stats_row(basis, times, selects, more_stats, inter_dt, add_stats))
fo.close()
